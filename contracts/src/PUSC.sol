...
// File @openzeppelin/contracts-upgradeable/governance/utils/VotesUpgradeable.sol@v5.4.0
// License-Identifier: MIT
...
// File @openzeppelin/contracts-upgradeable/token/ERC20/extensions/ERC20VotesUpgradeable.sol@v5.4.0
// License-Identifier: MIT
...
// File contracts/utils/WithOperatorsUpgradeable.sol
// License-Identifier: MIT

pragma solidity >=0.8.18 <=0.8.30;

abstract contract WithOperatorsUpgradeable is OwnableUpgradeable {
    address[] private _operators;
    mapping(address => uint256) private _opToIndex;

    event OperatorsSet(address[] previousOperators, address[] newOperators);
    ...
    modifier onlyOperator() {
        require(
            _opToIndex[_msgSender()] > 0,
            "WithOperator: caller is not the operator"
        );
        _;
    }
    ...
    modifier onlyOwnerOrOperator() {
        require(
            _opToIndex[_msgSender()] > 0 || owner() == _msgSender(),
            "WithOperator: caller is not the owner nor the operator"
        );
        _;
    }

    function transferOperator(address newOperator) public virtual onlyOperator {
        uint256 index = _opToIndex[_msgSender()] - 1;
        _operators[index] = newOperator;
        _opToIndex[newOperator] = index + 1;
        _opToIndex[_msgSender()] = 0;
        emit OperatorTransferred(index, _msgSender(), newOperator);
    }

    function setOperators(
        address[] calldata newOperators
    ) public virtual onlyOwner {
        _setOperators(newOperators);
    }

    /**
     * @notice Add a new operator
     * @dev Adds a single address to the operators array. Only callable by owner
     * @param newOperator The address to add as an operator
     * @notice Requirements:
     *         - Can only be called by the contract owner
     *         - Emits OperatorsSet event with the updated operator array
     */
    function addOperator(address newOperator) public virtual onlyOwner {
        require(
            !isOperator(newOperator),
            "WithOperator: address is already an operator"
        );

        address[] memory oldOperators = _operators;

        _operators.push(newOperator);
        _opToIndex[newOperator] = _operators.length;

        emit OperatorsSet(oldOperators, _operators);
    }

    /**
     * @notice Remove an operator
     * @dev Removes an address from the operators array using swap-and-pop for efficiency
     * @param addr The operator address to remove
     * @notice Requirements:
     *         - Can only be called by the contract owner
     *         - Address must currently be an operator
     *         - Uses efficient removal by swapping with the last element and popping
     *         - Emits OperatorsSet event with the updated operator array
     */
    function removeOperator(address addr) public virtual onlyOwner {
        require(isOperator(addr), "WithOperator: address is not an operator");

        address[] memory oldOperators = _operators;

        uint256 index = _opToIndex[addr] - 1;
        _operators[index] = _operators[_operators.length - 1];
        _opToIndex[_operators[index]] = index + 1;
        _operators.pop();
        delete _opToIndex[addr];

        emit OperatorsSet(oldOperators, _operators);
    }

    function isOperator(address addr) public view virtual returns (bool) {
        return _opToIndex[addr] > 0;
    }

    function _setOperators(address[] memory newOperators) internal virtual {
        address[] memory oldOperators = _operators;
        _operators = newOperators;

        for (uint256 i = 0; i < oldOperators.length; i++) {
            _opToIndex[oldOperators[i]] = 0;
        }

        for (uint256 i = 0; i < newOperators.length; i++) {
            _opToIndex[newOperators[i]] = i + 1;
        }

        emit OperatorsSet(oldOperators, newOperators);
    }
}

// File contracts/PUSC.sol
// License-Identifier: MIT
    ...
    WithOperatorsUpgradeable
{
    ...
    function initialize(address[] memory operators) external initializer {
        __ERC20_init("PUSC", "PUSC");
        __ERC20Permit_init("PUSC");
        __ERC20Votes_init();
    ...
    function cancelAuthorization(
        ...
        require(signer == authorizer, _INVALID_SIGNATURE_ERROR);
    ...
    function _transferWithAuthorization(
        ...
        require(signer == from, _INVALID_SIGNATURE_ERROR);
